...
- **新示例遵循既有纪律**：llmtest 剧本驱动保证 CI 可跑；工具构造失败启动即拒（fail-fast，不静默吞错）；每轮 Run 后等 `TitleFlight()` 收口再读终态或开下一轮（首轮收尾的异步标题生成走同一模型工厂、同样消耗剧本槽位——不等待则剧本错位；**例外**：经 submit_plan/审批挂起 + Resume 收尾的首轮无标题槽位——挂起时半轮 assistant 消息已 flushAcc 入史，settleTurn 的 firstTurn 判定为 false，标题恒回退 Task）；emit 回调不含 `user_message`（用户输入落流/订阅面才可见）。
