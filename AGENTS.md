...
- **钉 einox 最新发布版本**（go.mod require 已发布 tag，不用 replace 指向本地或 main）——公开代理可见的版本才是消费者实际拿到的。einox 发新版后升级依赖并按编译器提示修示例；先例：v0.1.1→v0.2.0 曾有 `NewManager` 返回值与 `Tools` 闭包签名两处漂移，随 v0.2.0 发布对齐、本仓同步升级。
